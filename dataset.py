...
class BirdDataset(data.Dataset):
    ...
    def __init__(self,root_img,root_mask,transform = None,target_transform = None):#root表示图片路径
         #os.listdir(path)返回指定路径下的文件和文件夹列表。/是真除法,//对结果取整
        imglist=os.listdir(root_img)
        masklist=os.listdir(root_mask)
        n=len(imglist)
        assert(len(masklist)==n),"len(masklist)!=n"
        imgs = []

        for i in range(n):

            img = os.path.join(root_img,imglist[i])#os.path.join(path1[,path2[,......]]):将多个路径组合后返回

            mask = os.path.join(root_mask,masklist[i])
    ...
    def __len__(self):
        ...
        return len(self.imgs)#200,list[i]有两个元素，[img,mask]
